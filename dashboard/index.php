...
if ($role === 'admin') {
    ...
    $daily_labels = []; $daily_values = []; $daily_map = [];
    for ($i = 29; $i >= 0; $i--) { $d = date('Y-m-d', strtotime("-$i days")); $daily_map[$d] = 0; }
    while ($row = mysqli_fetch_assoc($daily_chart)) { $daily_map[$row['day']] = (float)$row['total']; }
    foreach ($daily_map as $d => $v) { $daily_labels[] = date('d M', strtotime($d)); $daily_values[] = $v; }
    ...
    while ($row = mysqli_fetch_assoc($monthly_chart)) { $monthly_labels[] = date('M Y', strtotime($row['month'] . '-01')); $monthly_values[] = (float)$row['total']; }
    ...
    $profit_map = [];
    for ($i = 29; $i >= 0; $i--) { $d = date('Y-m-d', strtotime("-$i days")); $profit_map[$d] = 0; }
    while ($row = mysqli_fetch_assoc($profit_chart)) { $profit_map[$row['day']] = (float)$row['profit']; }
    ...
    $purchase_monthly_labels = []; $purchase_monthly_values = [];
    $purchase_map = [];
    for ($i = 11; $i >= 0; $i--) { $m = date('Y-m', strtotime("-$i months")); $purchase_map[$m] = 0; }
    while ($row = mysqli_fetch_assoc($purchase_monthly)) { $purchase_map[$row['month']] = (float)$row['total']; }
    foreach ($purchase_map as $m => $v) { $purchase_monthly_labels[] = date('M Y', strtotime($m . '-01')); $purchase_monthly_values[] = $v; }
...
if ($role === 'staff') {
    ...
    $staff_pch_labels = []; $staff_pch_values = [];
    while ($row = mysqli_fetch_assoc($purchase_chart_staff)) { $staff_pch_labels[] = date('M Y', strtotime($row['month'] . '-01')); $staff_pch_values[] = (float)$row['total']; }
}
...
<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-6">
    <?= statCard(
        ...
        'bg-indigo-100', 'text-indigo-600', $total_products, 'Total Products'
    ) ?>
    <?= statCard(
        ...
        'bg-blue-100', 'text-blue-600', $total_categories, 'Categories', 'stagger-1'
    ) ?>
    <?= statCard(
        ...
        'bg-emerald-100', 'text-emerald-600', $total_suppliers, 'Suppliers', 'stagger-2'
    ) ?>
    <?= statCard(
        ...
        'bg-amber-100', 'text-amber-600', $total_customers, 'Customers', 'stagger-3'
    ) ?>
    <?= statCard(
        ...
        'bg-purple-100', 'text-purple-600', $total_users, 'Users', 'stagger-4'
    ) ?>
    <?= statCard(
        '<svg class="w-4.5 h-4.5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>',
        'bg-green-100', 'text-green-600', number_format($today_sales['revenue']), "Today's Revenue", 'stagger-5'
    ) ?>
    <?= statCard(
        ...
        'bg-rose-100', 'text-rose-600', number_format($total_revenue), 'Monthly Revenue', 'stagger-6'
    ) ?>
    <?= statCard(
        '<svg class="w-4.5 h-4.5 text-cyan-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>',
        'bg-cyan-100', 'text-cyan-600', number_format(max(0, $total_profit)), 'Total Profit', 'stagger-7'
    ) ?>
    <?= statCard(
        ...
        'bg-red-100', 'text-red-600', $low_stock_count, 'Low Stock Items', 'stagger-8'
    ) ?>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    <div class="card fade-in">
        <div class="card-header"><h2 class="text-base font-bold text-gray-900">Daily Sales (30 Days)</h2></div>
        <div class="card-body"><canvas id="dailySalesChart" height="120"></canvas></div>
    </div>
    <div class="card fade-in stagger-1">
        <div class="card-header"><h2 class="text-base font-bold text-gray-900">Monthly Sales</h2></div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    <div class="card fade-in">
        <div class="card-header"><h2 class="text-base font-bold text-gray-900">Profit Trend (30 Days)</h2></div>
        <div class="card-body"><canvas id="profitChart" height="120"></canvas></div>
    </div>
    <div class="card fade-in stagger-1">
        <div class="card-header"><h2 class="text-base font-bold text-gray-900">Top Selling Products</h2></div>
        ...
        <div class="card-body p-0">
            ...
            <div class="overflow-x-auto">
                <table class="data-table">
                    <thead>
                        <tr><th>Invoice</th><th>Date</th><th>Customer</th><th>Cashier</th><th class="text-right">Amount</th></tr>
                    </thead>
                    <tbody>
            ...
            <?php else: ?>
            <div class="empty-state">
                <svg class="w-12 h-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z"/></svg>
                <h3>No sales yet</h3>
            </div>
            <?php endif; ?>
        </div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
    <div class="lg:col-span-2 card fade-in">
        ...
        <div class="card-body p-0">
            ...
            <div class="overflow-x-auto">
                <table class="data-table">
                    <thead>
                        <tr><th>Invoice</th><th>Supplier</th><th>Date</th><th class="text-right">Amount</th><th>Status</th></tr>
                    </thead>
                    <tbody>
    ...
    <div class="card fade-in stagger-1">
        <div class="card-header"><h2 class="text-base font-bold text-gray-900">Monthly Purchases</h2></div>
...
<div class="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-6">
    <div class="card fade-in">
        <div class="card-header"><h2 class="text-base font-bold text-gray-900">Forecast Summary</h2></div>
        <div class="card-body">
            <div class="space-y-4">
    ...
    <div class="lg:col-span-3 card fade-in stagger-1">
        ...
        <div class="card-body p-0">
            ...
            <div class="overflow-x-auto">
                <table class="data-table">
                    <thead>
                        <tr><th>Product</th><th>Forecast Date</th><th>Forecast Qty</th><th>Demand Level</th><th>Recommended Stock</th></tr>
                    </thead>
                    <tbody>
...
<div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
    <a href="../product/index.php" class="card fade-in p-4 flex items-center gap-3 hover:shadow-md transition-shadow">
        <div class="w-10 h-10 bg-indigo-100 rounded-lg flex items-center justify-center"><svg class="w-5 h-5 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg></div>
        <div><p class="font-semibold text-gray-900 text-sm">Add Product</p><p class="text-xs text-gray-500">New inventory item</p></div>
    </a>
    ...
    <a href="../reports/index.php" class="card fade-in stagger-3 p-4 flex items-center gap-3 hover:shadow-md transition-shadow">
        ...
        <div><p class="font-semibold text-gray-900 text-sm">Reports</p><p class="text-xs text-gray-500">View analytics</p></div>
    </a>
</div>
...
<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
    ...
    <?= statCard(
        ...
        'bg-amber-100', 'text-amber-600', $stock_summary['total_qty'], 'Total Stock Qty', 'stagger-3'
    ) ?>
    <?= statCard(
        '<svg class="w-4.5 h-4.5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z"/></svg>',
        'bg-red-100', 'text-red-600', $low_stock_count, 'Low Stock Items', 'stagger-4'
    ) ?>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
    <div class="card fade-in">
        ...
        <div class="card-body">
            <?php mysqli_data_seek($low_stock_result, 0); if ($low_stock_result && mysqli_num_rows($low_stock_result) > 0): while ($p = mysqli_fetch_assoc($low_stock_result)): ?>
            <div class="flex items-center justify-between p-3 rounded-lg mb-2 <?= $p['quantity'] == 0 ? 'bg-red-50' : 'bg-amber-50' ?>">
                <div>
                    <p class="font-semibold text-gray-800 text-sm"><?= htmlspecialchars($p['product_name']) ?></p>
                    <p class="text-xs text-gray-500"><?= htmlspecialchars($p['category_name'] ?? 'Uncategorized') ?></p>
    ...
    <!-- Stock Summary -->
    <div class="card fade-in stagger-1">
        <div class="card-header"><h2 class="text-base font-bold text-gray-900">Stock Summary</h2></div>
        <div class="card-body">
            <div class="space-y-4">
                <div class="flex items-center justify-between">
    ...
    <div class="card fade-in stagger-2">
        <div class="card-header"><h2 class="text-base font-bold text-gray-900">Purchase Summary</h2></div>
        <div class="card-body">
            <div class="space-y-4">
                <div class="flex items-center justify-between">
...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
    <?= statCard(
        ...
        'bg-blue-100', 'text-blue-600', $today_data['count'], "Today's Transactions"
    ) ?>
    <?= statCard(
        '<svg class="w-4.5 h-4.5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>',
        'bg-green-100', 'text-green-600', number_format($today_data['revenue']), "Today's Revenue", 'stagger-1'
    ) ?>
    <?= statCard(
        ...
        'bg-indigo-100', 'text-indigo-600', $total_products, 'Products Available', 'stagger-2'
    ) ?>
</div>
...
<div class="card fade-in">
    ...
    <div class="card-body p-0">
        ...
        <div class="overflow-x-auto">
            <table class="data-table">
                <thead>
                    <tr><th>Invoice</th><th>Date</th><th>Customer</th><th class="text-right">Amount</th></tr>
                </thead>
                <tbody>
                    <?php while ($s = mysqli_fetch_assoc($recent_sales)): ?>
                    <tr>
...
new Chart(document.getElementById('dailySalesChart'), {
    ...
    data: {
        ...
        datasets: [{
            label: 'Revenue', data: <?= json_encode($daily_values) ?>,
            borderColor: '#4f46e5', backgroundColor: 'rgba(79, 70, 229, 0.08)',
            fill: true, tension: 0.4, pointRadius: 2, pointBackgroundColor: '#4f46e5'
        }]
    },
    options: {
        responsive: true, maintainAspectRatio: true,
        plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => 'Revenue: ' + Number(ctx.raw).toLocaleString() + ' Ks' } } },
        scales: { y: { beginAtZero: true, grid: { color: 'rgba(0,0,0,0.04)' }, ticks: { callback: v => v.toLocaleString() } }, x: { grid: { display: false }, ticks: { maxTicksLimit: 15 } } }
    }
});
...
new Chart(document.getElementById('monthlySalesChart'), {
    ...
    data: {
        ...
        datasets: [{
            label: 'Monthly Sales', data: <?= json_encode($monthly_values) ?>,
            backgroundColor: 'rgba(99, 102, 241, 0.2)', borderColor: '#6366f1', borderWidth: 2, borderRadius: 6, barPercentage: 0.6
        }]
    },
    options: {
        responsive: true, maintainAspectRatio: true,
        plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => 'Sales: ' + Number(ctx.raw).toLocaleString() + ' Ks' } } },
        scales: { y: { beginAtZero: true, grid: { color: 'rgba(0,0,0,0.04)' }, ticks: { callback: v => v.toLocaleString() } }, x: { grid: { display: false } } }
    }
});
...
new Chart(document.getElementById('profitChart'), {
    ...
    data: {
        labels: <?= json_encode($daily_labels) ?>,
        datasets: [{
            label: 'Profit', data: <?= json_encode($profit_values) ?>,
            borderColor: '#059669', backgroundColor: 'rgba(5, 150, 105, 0.08)',
            fill: true, tension: 0.4, pointRadius: 2, pointBackgroundColor: '#059669'
        }]
    },
    options: {
        responsive: true, maintainAspectRatio: true,
        plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => 'Profit: ' + Number(ctx.raw).toLocaleString() + ' Ks' } } },
        scales: { y: { beginAtZero: true, grid: { color: 'rgba(0,0,0,0.04)' }, ticks: { callback: v => v.toLocaleString() } }, x: { grid: { display: false }, ticks: { maxTicksLimit: 15 } } }
    }
});
...
new Chart(document.getElementById('topProductsChart'), {
    ...
    data: {
        ...
        datasets: [{
            label: 'Quantity Sold', data: <?= json_encode($top_qty) ?>,
            ...
            borderColor: '#4f46e5', borderWidth: 1, borderRadius: 4
        }]
    },
    options: {
        responsive: true, maintainAspectRatio: true, indexAxis: 'y',
        plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => 'Sold: ' + Number(ctx.raw).toLocaleString() + ' units' } } },
...
new Chart(document.getElementById('purchaseChart'), {
    ...
    data: {
        ...
        datasets: [{
            label: 'Purchases', data: <?= json_encode($purchase_monthly_values) ?>,
            backgroundColor: 'rgba(16, 185, 129, 0.2)', borderColor: '#10b981', borderWidth: 2, borderRadius: 6, barPercentage: 0.6
        }]
    },
    options: {
        responsive: true, maintainAspectRatio: true,
        plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => 'Purchases: ' + Number(ctx.raw).toLocaleString() + ' Ks' } } },
        scales: { y: { beginAtZero: true, grid: { color: 'rgba(0,0,0,0.04)' }, ticks: { callback: v => v.toLocaleString() } }, x: { grid: { display: false } } }
    }
});
<?php elseif ($role === 'staff'): ?>
...
new Chart(document.getElementById('staffPurchaseChart'), {
    ...
    data: {
        ...
        datasets: [{
            label: 'Purchases', data: <?= json_encode($staff_pch_values) ?>,
    ...
    options: {
        responsive: true, maintainAspectRatio: true,
        plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => Number(ctx.raw).toLocaleString() + ' Ks' } } },
